...
// Image-less mode: when no `src` is provided the composite pass skips the
// photo entirely and draws the displacement field itself as glowing ripple
// rings (tinted body + bright sheen rims) on a fully transparent canvas —
// a pure cursor effect to lay over any background.
const rippleOnlyFragment = `
precision highp float;
...
void main() {
  ...
  float ex = texture2D(uDisplacement, vUv + vec2(uTexel.x, 0.0)).r - texture2D(uDisplacement, vUv - vec2(uTexel.x, 0.0)).r;
  float ey = texture2D(uDisplacement, vUv + vec2(0.0, uTexel.y)).r - texture2D(uDisplacement, vUv - vec2(0.0, uTexel.y)).r;
  float ridge = clamp(length(vec2(ex, ey)) * 22.0, 0.0, 1.0);

  vec3 color = mix(uTint * 1.9, uHighlight, ridge);
  float alpha = clamp(amount * 1.15, 0.0, 1.0) * clamp(uTintAmount * 4.0, 0.0, 1.0);
  gl_FragColor = vec4(color, alpha * (0.35 + 0.65 * ridge));
}
`;
...
const RippleDistortion = ({
  ...
  useEffect(() => {
    // Keep the live config the WebGL loop / handlers read from.
    // (Declared before the main effect so it always runs first.)
  ...
  useEffect(() => {
    ...
    // Image-less mode: no photo is distorted — the displacement field itself
    ...
    const compositeMesh = new Mesh(gl, {
      ...
      program: new Program(gl, {
        vertex: screenVertex,
        fragment: rippleOnly ? rippleOnlyFragment : compositeFragment,
        uniforms: compositeUniforms,
        transparent: rippleOnly,
        depthTest: false,
        depthWrite: false
      })
    });
